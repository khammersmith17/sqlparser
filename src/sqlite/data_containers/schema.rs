...
#[derive(Debug, Clone)]
pub enum IndexPrimaryKey {
    RowId,
    Simple,
    Composite(usize), // length primary key
    None,
}

impl IndexPrimaryKey {
    pub fn len(&self) -> usize {
        match self {
            Self::RowId | Self::Simple => 1_usize,
            Self::None => 0_usize,
            Self::Composite(len) => *len,
        }
    }
}
...
impl TableMetadata {
    pub fn has_row_id(&self) -> bool {
        !self.table_schema.without_rowid
    }

    pub fn table_column_names(&self) -> Vec<&str> {
        self.table_schema.column_names()
    }
}
...
pub struct IndexMetadata {
    ...
    #[allow(dead_code)]
    pub index_schema: IndexSchema,
    pub table_primary_key: IndexPrimaryKey,
}
...
impl IndexMetadata {
    ...
    pub fn available_columns(&self) -> &[String] {
        self.available_columns.as_ref()
    }

    pub fn row_id_column_name(&self) -> Option<&str> {
        match self.table_primary_key {
            IndexPrimaryKey::Simple | IndexPrimaryKey::RowId => {
                Some(self.available_columns[0].as_ref())
            }
            IndexPrimaryKey::None => None,
            IndexPrimaryKey::Composite(_) => todo!(),
        }
    }
}
...
impl IndexSchema {
    pub fn new(schema: String) -> Result<(String, IndexSchema)> {
...
impl SqliteSchema {
    pub fn new(file: &mut File) -> Result<SqliteSchema> {
        ...
        for offset in cell_offsets {
            ...
            let (header_bytes_read, schema_record_header) =
                SchemaTableRecordHeader::new(offset.into(), file)?;

            let SchemaTableRecordHeader {
                record_header_size,
        ...
        for index in temp_indexes.into_iter() {
            ...
            let (bit_mask, available_columns, index_pk) =
                match base_table_ref.table_schema.primary_key {
                    PrimaryKey::None => {
                    ...
                    PrimaryKey::Simple(i) => {
                        ...
                        (
                            generate_column_bitmask(&base_table_ref.column_bitmap, &cols_w_pk),
                            cols_w_pk,
                            IndexPrimaryKey::Simple,
                        )
                    }
                    PrimaryKey::Composite(ref pk_set) => {
                        let primary_key = IndexPrimaryKey::Composite(pk_set.len());
                        let mut cols_w_pk: Vec<String> =
                            Vec::with_capacity(index_schema.index_columns.len() + pk_set.len());
                        for i in pk_set.iter() {
                            cols_w_pk
                                .push(base_table_ref.table_schema.columns[*i].name.to_uppercase());
                        }
                        for col in &index_schema.index_columns {
                            cols_w_pk.push(col.name.to_uppercase())
                        }
    ...
    pub fn resolve_query_metadata(
        ...
        let required_bitmask =
            generate_column_bitmask(&base_table.column_bitmap, evaluated_columns);
...
pub enum IndexRowRef {
    RowId(i64),
    PrimaryKeyTuple(Vec<Value>),
}

impl PartialOrd for IndexRowRef {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match self {
            Self::RowId(my_row_id) => match other {
                Self::RowId(other_row_id) => Some(my_row_id.cmp(other_row_id)),
                Self::PrimaryKeyTuple(_) => None,
            },
            Self::PrimaryKeyTuple(my_value) => match other {
                Self::RowId(_) => None,
                Self::PrimaryKeyTuple(other_value) => {
                    // iterate until one there are not equal
                    let min_len = std::cmp::min(my_value.len(), other_value.len());

                    let mut i = 0_usize;

                    while i < min_len {
                        if let Some(cmp) = my_value[i].partial_cmp(&other_value[i]) {
                    ...
                    // If the overlapping values are equal, shorter one wins
                    match (my_value.len() as i32) - (other_value.len() as i32) {
                        0_i32 => Some(Ordering::Equal),
...
struct SchemaTableRecordHeader {
...
impl SchemaTableRecordHeader {
    fn new(offset: u64, file: &mut File) -> Result<(i64, SchemaTableRecordHeader)> {
        file.seek(std::io::SeekFrom::Start(offset))?;

        let _ = read_varint(file)?;
        ...
        let SqliteVarint {
            varint: schema_size,
            byte_size: schema_size_bytes_read,
        } = read_varint(file)?;
        ...
        Ok((
            ...
            SchemaTableRecordHeader {
                record_header_size: record_header_size as i64,
                schema_type_serial_type,
                schema_name_serial_type,
                table_name_serial_type,
                root_page_serial_type,
